...
import { MarkdownString, Range, TextEditor } from "vscode";
...
interface EvaluateSelectionProvider {
  evaluateString(
    ...
    range: Range
  ): vscode.ProviderResult<EvaluateSelectionRequest.EvaluateSelectionResult>;
}
...
interface EvaluateSelectionRegistrationOptions
  extends EvaluateSelectionOptions,
    TextDocumentRegistrationOptions,
    StaticRegistrationOptions {}
...
async function evaluateString(
  ...
  document: vscode.TextDocument,
  range: Range
): Promise<EvaluateSelectionRequest.EvaluateSelectionResult> {
  const activeTextEditor = vscode.window.activeTextEditor;
  ...
  registerLanguageProvider(): [vscode.Disposable, EvaluateSelectionProvider] {
    const provider: EvaluateSelectionProvider = {
      evaluateString: (document: vscode.TextDocument, range: Range) => {
        ...
        const provideEvaluateSelection = (
          document: vscode.TextDocument,
          range: Range
        ) => {
